...
public class MovementSystem {
	...
	public void move(ScannedRobotEvent e) {
		...
		if (Math.random() > .9) {
			dagge.setMaxVelocity((12 * Math.random()) + 24); // randomly change
																// speed
		}

		if (e.getDistance() > 150) {
			dagge.setTurnRightRadians(robocode.util.Utils
					.normalRelativeAngle(absBearing - dagge.getHeadingRadians() + latVel / dagge.getVelocity())); // drive
																													// towards
																													// the
																													// enemies
																													// predicted
																													// future
																													// location
			dagge.setAhead((e.getDistance() - 140) * (moveDirection / Math.abs(moveDirection))); // move
																									// forward

		} else {
			dagge.setTurnLeft(-90 - e.getBearing()); // turn perpendicular to
														// the enemy
			dagge.setAhead(Math.max((e.getDistance() - 140), 20) * (moveDirection / Math.abs(moveDirection))); // move
																												// forward
		}
		moveDirection++;
