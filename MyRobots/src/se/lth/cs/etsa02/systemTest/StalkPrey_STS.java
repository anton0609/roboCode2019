...
public class StalkPrey_STS extends RobotTestBed{
			...
			private double stalking;
			private double notStalking;
			private double goodRange;
			private double badRange;
			...
			public void onBattleCompleted(BattleCompletedEvent event) {
				System.out.println("Stalks prey " + 100 * stalking / (stalking + notStalking) + "% of all turns");
				System.out.println("Within optimal distance " + 100 * goodRange / (goodRange + badRange) + "% of all turns");
				assertTrue("Does not stalk ", 100 * stalking / (stalking + notStalking) > 50);
				assertTrue("Not clsoe enough ", 100 * goodRange / (goodRange + badRange) > 70);
			...
			public void onTurnEnded(TurnEndedEvent event) {
				IRobotSnapshot robot0 = event.getTurnSnapshot().getRobots()[0];
				IRobotSnapshot robot1 = event.getTurnSnapshot().getRobots()[1];
				
				currentDist = Math.hypot(robot0.getX() - robot1.getX(), robot0.getY() - robot1.getY());
				if(currentDist > 270 && prevDist >= currentDist) {
					stalking++;
				} else if(currentDist > 270){
					notStalking++;
					badRange++;
				}
				if(currentDist <= 270) {
