...
public class MovementSystem {
	...
	public void move(ScannedRobotEvent e) {
		...
		if (e.getDistance() > 220) {
			...
			if (left == 1 || right == 1 || up == 1 || down == 1) {
				...
				} else if (dagge.getHeadingRadians() >= 3 * Math.PI / 2) {
				...
				} else if (dagge.getHeadingRadians() >= Math.PI / 2 && dagge.getHeadingRadians() < Math.PI) {
				...
				} else if (dagge.getHeadingRadians() < 3 * Math.PI / 2 && dagge.getHeadingRadians() >= Math.PI) {
					dagge.setTurnLeftRadians(right * (dagge.getHeadingRadians() - Math.PI / 2));
					dagge.setTurnRightRadians(left * (Math.PI - dagge.getHeadingRadians() + Math.PI / 2));
					dagge.setTurnRightRadians(up * (2 * Math.PI - dagge.getHeadingRadians()));
					dagge.setTurnLeftRadians(down * (dagge.getHeadingRadians() - Math.PI));
	...
	/**
	 * Changes move direction of the robot and flags it as stuck
	 * 
	 * @param e
	 *            - The HitRobotEvent
	 */
	...
	/**
	 * Changes move direction for the robot and flags it as stuck
	 * 
	 * @param e
	 *            - The HitWallEvent
	 */
	...
	/**
	 * Getter for the moveDriection attribute
	 * 
	 * @return current moveDirection;
	 */
	...
	/**
	 * 
	 * @return a Double[2] with absolute bearing at index 0 and later velocit at
	 *         index 1;
	 */
